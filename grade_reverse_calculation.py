...
def subjectGradeCalculation(subject, gpa):
    ...
    print(f"Selected Subject Grade: {list(grade_translation.keys())[list(grade_translation.values()).index(selected_subject_grade)]}")
    print(f"Selected Subject Grade: {selected_subject_grade:.2f}")
...
while True:
    try:
        ...
        if selection < 1 or selection > len(grade_dict):
            raise ValueError("Invalid selection. Please enter a number between 1 and", len(grade_dict))
